...
const char * const BITCOIN_CONF_FILENAME = "eden.conf";
const char * const BITCOIN_PID_FILENAME = "edend.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "eden" is a composite category enabling all Eden-related debug output
            if(ptrCategory->count(string("eden"))) {
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty eden.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override eden.conf
